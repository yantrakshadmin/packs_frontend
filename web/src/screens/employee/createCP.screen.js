...
const CreateCPScreen = ({ currentPage }) => {
          ...
          <Button
            ...
            onClick={(e) => {
              navigate(`../print-cp/${record.id}`,{ state:{ ...record,receiverDetails:[{
                ...
                receiver_name:record.pfep.receivers.length>0?
                  record.pfep.receivers[0].name:'',
                receiver_location: record.pfep.receivers.length>0?
  ...
  const rowSelection = {
    ...
    getCheckboxProps: record => ({
      disabled: selectedCP.length<=0?false:
